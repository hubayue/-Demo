...
const output = join(outputDirectory, "v7.19.14-web-ruler-selection.png");
const session = `ruler-parity-${process.pid}`;
...
try {
  ...
  const state = await cli("eval", "() => { document.getElementById('acctBox')?.classList.add('hide'); newGame(makeWeekLevel(meta.week, 0)); state.phase = 'pickLord'; return { version: GAME_VERSION, phase: state.phase }; }");
  if (!state.includes('"version": "7.19.14"') || !state.includes('"phase": "pickLord"')) {
    throw new Error(`Unexpected ruler state: ${state}`);
  }
  ...
  process.stdout.write(`Captured Web v7.19.14 ruler selection to ${output}\n`);
} finally {
